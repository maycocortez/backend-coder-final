    ...
    new JWTStrategy(
      ...
      async (jwtPayload, done) => {
        try {
          ...
          if (Array.isArray(jwtPayload.roles) && jwtPayload.roles.includes('User')) {
  ...
  passport.use(
    ...
    new LocalStrategy(
      ...
      async (req, username, password, done) => {
        try {
          const { firstName, lastName, age, passwordConfirm, roles } = req.body;
          ...
          if (roles) {
            const searchRoles = await roleService.findRoles({
              name: { $in: roles },
            });
          ...
          const addCarts = await userService.addCartToUser();
          newUser.cart = addCarts;
